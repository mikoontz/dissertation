...
\begin{ucfrontmatter}
  ...
  \title{The effect of vegetation spatial structure on forest resilience to
wildfire and bark beetle disturbance in the Sierra Nevada, California}
  \author{Michael J. Koontz}

\report{DISSERTATION} 
  \degree{DOCTOR OF PHILOSOPHY} 
  \degreemonth{April} \degreeyear{2019}
...
\KeywordTok{library}\NormalTok{(aggiedown)}
\end{Highlighting}
\end{Shaded}
\clearpage
...
\href{https://github.com/rapeek/aggiedown/issues/new}{contact us} (email
\href{mailto:rapeek@ucdavis.edu}{\nolinkurl{rapeek@ucdavis.edu}}) with
your suggestions. We love to see people using \emph{R Markdown} for
their theses, so we'll do our best to help.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# This chunk ensures that the aggiedown package is}
\CommentTok{# installed and loaded. This aggiedown package includes}
\CommentTok{# the template files for the thesis.}
\ControlFlowTok{if}\NormalTok{(}\OperatorTok{!}\KeywordTok{require}\NormalTok{(devtools))}
  \KeywordTok{install.packages}\NormalTok{(}\StringTok{"devtools"}\NormalTok{, }\DataTypeTok{repos =} \StringTok{"http://cran.rstudio.com"}\NormalTok{)}
\ControlFlowTok{if}\NormalTok{(}\OperatorTok{!}\KeywordTok{require}\NormalTok{(aggiedown))}
\NormalTok{  devtools}\OperatorTok{::}\KeywordTok{install_github}\NormalTok{(}\StringTok{"ryanpeek/aggiedown"}\NormalTok{)}
\KeywordTok{library}\NormalTok{(aggiedown)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  devtools}\OperatorTok{::}\KeywordTok{install_github}\NormalTok{(}\StringTok{"rapeek/aggiedown"}\NormalTok{)}
\NormalTok{  \}}
\KeywordTok{library}\NormalTok{(aggiedown)}
\NormalTok{flights <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"data/flights.csv"}\NormalTok{)}
\end{Highlighting}
...
\href{https://github.com/ryanpeek/aggiedown}{aggiedown} and
\href{https://github.com/rstudio/bookdown}{bookdown}.

This document was typeset using the XeTeX typesetting system, and the
University of California Thesis class. Under the hood, the elements of
...
in the University of XXX library.

This version of the thesis was generated on 2019-04-16 18:14:55. The
...
- Packages --------------------------------------------------------------
 ...
 aggiedown   * 1.0     2019-04-16 [1] Github (ryanpeek/aggiedown@ae99300)
