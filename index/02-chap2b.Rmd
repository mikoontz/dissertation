# Differential response of a tree-killing bark beetle to forest structure across a gradient of climatic water deficit
<!-- This adds a different "short title" -->
<!-- \chaptermark{Local structure; western pine beetle severity} -->
Michael J. Koontz^1,2,\*^, Andrew M. Latimer^1,2^, Leif A. Mortenson^3^, Christopher J. Fettig^3^, Malcolm P. North^1,2,4^
...
```{r setup_chap02, include = FALSE, echo = FALSE}
...
eq_nums_chap02 <- captioner(prefix = "Equation 2.", auto_space = FALSE)
```

```{r source_necessary_scripts}
# Information about correspondence of tree top detection algorithms with ground data
algorithm_stats <- read_rds("analyses_output/chap02/algorithm_stats.rds")
...
d <- read_csv("analyses_output/chap02/formatted-ground-data.csv")
```
...
ground_tree_summary <- readr::read_csv("analyses_output/chap02/ground-tree-summary.csv")
...
```{r classifier_stats}
classifier_stats <- read_csv("analyses_output/chap02/classification-summary-stats.csv")
```

```{r hand_classified_crowns}
hand_classified_crowns <- read_csv("analyses_output/chap02/hand-classified-crowns.csv") %>% 
  dplyr::mutate(site = substr(treeID, start = 1, stop = 9))
```

```{r site_characteristics_read_file}
site_characteristics <- read_csv("analyses_output/chap02/summarized-non-spatial-site-data.csv")
```
...
```{r}
table_nums_chap02(name = "table-site-deets")
```
...
site_characteristics <- 
  ...
  dplyr::mutate(`% tree mortality\n(aerial/ground)` = paste(sprintf("%0.0f", prop_mortality * 100), sprintf("%0.0f", ground_mortality * 100), sep = "/"),
                `Density\n(tpha; aerial/ground)` = paste(sprintf("%0.0f", air_tpha), sprintf("%0.0f", ground_tpha), sep = "/")) %>% 
  dplyr::rename(`CWD\n(mm)` = site_cwd,
...
### Tree detection
We found that the experimental `lmfx` algorithm with parameter values of `dist2d = `r algorithm_details[1, "var1_val"]`` and `ws = `r algorithm_details[1, "var2_val"]`` [@roussel2019] performed the best across 7 measures of forest structure as measured by Pearson's correlation with ground data (`r table_nums_chap02(name = "table-best-algorithm-deets", display = "cite")`).

```{r}
table_nums_chap02(name = "table-best-algorithm-deets")
```
